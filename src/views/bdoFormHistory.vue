    ...
    <table style="width: 95%; margin-top: 20px" class="table">
      <thead>
        <tr style="background: #b8affa">
          ...
          <th>Name of Beneficiary Bank</th>
...
export default {
  ...
  watch: {
    ...
    $route: {
      immediate: true,
      handler(to) {
        document.title = to.meta.title || "HISTORY BDO";
      },
    },
  },
  data() {
  ...
  methods: {
    openHistory(id, name) {
      localStorage.setItem("bdoglobalhistoryListId", id);
      localStorage.setItem("bdoglobalhistoryListName", name);
    ...
    editHistory(id, name) {
      localStorage.setItem("bdoglobalhistoryListId", id);
      localStorage.setItem("bdoglobalhistoryListName", name);
      localStorage.setItem("bdoglobalhistoryListUpdate", true);
      this.$router.push("/bdo");
    },
